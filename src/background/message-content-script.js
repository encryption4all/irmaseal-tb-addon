const showNotification = async () => {
    const mailDetails = await browser.runtime.sendMessage({
        command: 'queryMailDetails',
    })

    const { sealed, sender, identity, messageId } = mailDetails
    ...
    decryptButton.addEventListener('click', async () => {
        const sessionDetails = await browser.runtime.sendMessage({
            command: 'startSession',
            args: { messageId: messageId },
        })
        console.log('sessionDetails: ', sessionDetails)
        const qr = document.createElement('img')
        qr.src = sessionDetails.qrData
        envelope.insertBefore(qr, decryptButton)
